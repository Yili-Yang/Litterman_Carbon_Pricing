function [TABLE1,TABLE2,TABLE3,TABLE4] = run_global_RBF_FM(problem,probstop,gr_phase2,useg)
% four method
...
%TABLE1 - - original method, quasi-newton
%TABLE2 - - AD forward
%TABLE3 - - AD reverse
...
f_ph22=zeros(probstop -problem+1,1);
it_ph22=zeros(probstop -problem+1,1);
gNum2=zeros(probstop -problem+1,1);
gNorm2=zeros(probstop -problem+1,1);
f2_value2=zeros(probstop -problem+1,1);
% 
...
f_ph24=zeros(probstop -problem+1,1);
it_ph24=zeros(probstop -problem+1,1);
gNum4=zeros(probstop -problem+1,1);
gNorm4=zeros(probstop -problem+1,1);
f2_value4=zeros(probstop -problem+1,1);
...
for k=problem:probstop
    ...
    if gr_phase2 ==1 % apply quasi-Newton starting at xmin1
        ...
        [fmin22,xmin22,fcount22,gcount2]=ADforward_QN(myfun,xmin1,varargin);
      [fmin23,xmin23,fcount23,gcount3]=ADreverse_QN(myfun,xmin1,varargin); 
        ...
        [xmin24,fmin24,flag1,output1] = fminunc(myfun,xmin1,options,varargin);
        fcount24=output1.iterations;
        gcount4=output1.funcCount;
[fmin211,~,~,~]=quasi_newton(myfun,x(ind,:)',varargin);
 [fmin212,~,~,~]=ADforward_QN(myfun,x(ind,:)',varargin);
...
[~, g] = feval(myfun1, xmin22,varargin,options);
gNorm2(ic)= norm(g);
f2_value2(ic)= fmin212;
...
it_ph24(ic)= fcount24;
gNum4(ic)= gcount4;
[~,g]=feval(myfun,xmin24,varargin);
gNorm4(ic)= norm(g);
% f2_value4(ic)= fmin214;
%     
...
TABLE2 = table(ProbNo,fname,Probdim,f_init,f_ph1,it_ph1,f_ph22,...
    it_ph22,gNum2,gNorm2,f2_value2,...
    'VariableNames',{'ProbNo','fname','Probdim','f_init',...
    'f_ph1','it_ph1','f_ph2','it_ph2','gNum','gNorm','f2_value'});
...
TABLE4 = table(ProbNo,fname,Probdim,f_init,f_ph1,it_ph1,f_ph24,...
    it_ph24,gNum4,gNorm4,f2_value4,...
    'VariableNames',{'ProbNo','fname','Probdim','f_init',...
    'f_ph1','it_ph1','f_ph2','it_ph2','gNum','gNorm','f2_value'});