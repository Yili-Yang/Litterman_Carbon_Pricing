function [z, grad, s, Extra] = newtonst(functions, x, dependency, Extra, lambda)
% requires ADMAT methods: feval, evalj, evalh
% - - - - - - - - - - - - - - - - - - - - - - - - -
% newtonst computes the newton step s = -(H + lambda*I)^-1/grad without forming the Hessian explicitly
...
% lambda - parameter that controls trust region size
...
%		If sparsity information is not provided, the function will computed and return it
%		as Extra.JPIs
    %% initialization
    ...
    H = sparse(xDim+2*yDim,xDim+2*yDim);
	...
	for n = 1:(N_functions-1)
        ...
        idr = idx_y{n}; idc = [1:(xDim*dependency{n,1}), xDim+cell2mat(idx_y(dependency{n,2}))];
        H(idr,idc) = Y_J{n};
    end
    H((end-xDim+1):end,(end-yDim+1):end) = H(1:yDim,1:xDim)';
    H((yDim+1):2*yDim,(end-yDim+1):end) = H(1:yDim,xDim+(1:yDim))';
    [Y{end}, Y_J{end}, z_H] = evalh(functions{end}, input(x,Y,dependency(end,:)), Extra, HPIs{end});
    z = Y{end};
    H(1:2*yDim,(xDim+1):end) = H(1:2*yDim,(xDim+1):end) - speye(2*yDim);
    ...
    H((end-xDim+1):end,1:(end-yDim)) = g_H(1:xDim,:);
    H((yDim+1):2*yDim,1:(end-yDim)) = g_H((xDim+1):end,:);
    if lambda ~= 0
        H((end-xDim+1):end,1:xDim) = H((end-xDim+1):end,1:xDim) + lambda*speye(xDim);
    end
    b = sparse(xDim+2*yDim,1);
    b((end-xDim+1):end) = -grad';
    s = H\b;
    s = s(1:xDim);
    
    %% compute Hessian for testing purpose
    %B = H((2*yDim+1):end,1:xDim);
    %M = H((2*yDim+1):end,(xDim+1):end);
    %L = H(1:2*yDim,(xDim+1):end);
    %A = H(1:2*yDim,1:xDim);
    %Hess = B - M*(L\A);
end
