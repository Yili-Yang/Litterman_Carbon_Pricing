...
%
%
%   04/2007 -- reorganize the program for readibility
%   04/2007 -- correct the derivative computation
%
%       ******************************************************************
%       *                          ADMAT - 2.0                           *
...
elseif size(tape(i).val,1)==1 ||size(tape(i).val,2)==1
    ...
    if length(y)==1                        % y is a scalar
        ...
        if ~isempty(I1) 
            for j=1:length(I1) 
                tape(tape(i).arg2vc).W(I1(j),I1j(j),:) = ...
                    tape(tape(i).arg2vc).W(I1(j),I1j(j),:)+ ...
                     tape(i).W(I1(j),I1j(j),:);
            end
        end
    ...
    elseif length(x)==1                   % x is a scalar
        ...
        if ~isempty(I3)
            for j=1:length(I3) 
                tape(tape(i).arg1vc).W(I3(j),I3j(j),:)= ...
                    tape(tape(i).arg1vc).W(I3(j),I3j(j),:) + ...
                    tape(i).W(I3(j),I3j(j),:); 
            end
        end
