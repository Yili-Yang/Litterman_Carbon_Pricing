        ...
        if length(s1.val)==1               % s1.val is a scalar
            ...
            for j=1:length(I2)
                if isequal(squeeze(s2.derivH(I2(j),I2j(j))),s1.derivH)
                    sout.derivH(I2(j),I2j(j))= s1.derivH;
                else
                    error('Nondifferentiable points in max() was detected.');
                end
        ...
        elseif length(s2.val)==1       % s2.val is a scalar
            ...
            for j=1:length(I2)
                if isequal(squeeze(s1.derivH(I2(j),I2j(j))),s2.derivH)
                    sout.derivH(I2(j),I2j(j))= s2.derivH;
                else
                    error('Nondifferentiable points in max() was detected.');
                end
            ...
            if ~isempty(I2)
                for j=1: length(I2)
                    if isequal(s1.derivH(I2(j),I2j(j)),s2.derivH(I2(j),I2j(j)))
                        sout.derivH(I2(j),I2j(j))= s1.derivH(I2(j),I2j(j));
                    else
                        error('Nondifferentiable points in max() was detected.');
                    end
