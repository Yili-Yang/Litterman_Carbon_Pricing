...
if nargin > 1
    ...
    s2=derivtapeH(s2,0);
    sout=derivtapeH(min(s1.val,s2.val),0);
    ...
    if size(getval(sout),1)==1 || size(getval(sout),2)==1
        ...
        elseif length(getval(s2))==1 && length(getval(s1))~=1 
            ...
            if ~isempty(I2) 
                sout.deriv(I2,:)=(s1.deriv(I2,:)+s2.deriv(I2,:))/2; 
            end;
        end
        ...
        if length(s1.val)==1
            ...
            for j=1:length(I2) 
                sout.deriv(I2(j),I2j(j),:)= ...
                    (squeeze(s2.deriv(I2(j),I2j(j),:))+s1.deriv(:))/2; 
            end
        elseif length(s2.val)==1
            ...
            for j=1:length(I2)
                sout.deriv(I2(j),I2j(j),:)= ...
                    (squeeze(s1.deriv(I2(j),I2j(j),:))+s2.deriv(:))/2;
            end
        else
            sout.deriv=s2.deriv;
            if ~isempty(I1) 
