...
n = length(x);
if isa(x, 'derivspj')      % x is an objective of derivspj class
    ...
    % set y as an objective of derivspj class
    ydot = spones(ydot);
    y =derivspj(y, ydot);
elseif isa(x, 'deriv')  % x is an objective of derivspj class
     val = getval(x);    % get the value of x
    drv = getydot(x);   % get the derivative part of x
    y = mexbroy(val);   % compute the function value at x
    ydot = zeros(getval(n),globp);  % initialize the derivative of y
