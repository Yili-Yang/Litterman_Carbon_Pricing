...
class EZUtility(object):
	...
	potential_cons = np.ones(self.decision_times.shape) + self.cons_growth
	potential_cons = self.potential_cons ** self.decision_times
	add_penalty_cost = add_penalty_cost
	...
	def __init__(self, tree, damage, cost, period_len, eis=0.9, ra=7.0, time_pref=0.005, 
		...
		self.damage = damage
		self.cost = cost
		# periods (5 in the base) and decision_times(time when you make mitigation, array like[0,15,30,45,70])
		self.period_len = period_len
		self.decision_times = tree.decision_times
		# constant growth rate which is predefined (2% in the paper)
		...
		self.potential_cons = self.potential_cons ** self.decision_times # add the growth to every possible path
		# paras in the paper
		self.r = 1.0 - 1.0/eis #rau in paper 
		self.a = 1.0 - ra # alpha in paper
		self.b = (1.0-time_pref)**period_len # beta
	...
	def _end_period_marginal_utility(self, mu_tree_0, mu_tree_1, ce_tree, utility_tree, cons_tree):
		...
		# the utility of the final state can be separated into two parts: the part that is generated by consumption on T and the part is generated in the future.
