...
class GradientSearch(object) :
	...
	def _partial_grad_cons(self, i,cons):
		"""Calculate the ith element of the gradient vector."""
		m_copy = self.m.copy()
		m_copy[i] = m_copy[i] - self.delta if (m_copy[i] - self.delta)>=0 else 0.0
		minus_utility = self.u.adjusted_utility(m_copy,first_period_consadj=cons)
		m_copy[i] += 2*self.delta
		plus_utility = self.u.adjusted_utility(m_copy,first_period_consadj=cons)
		grad = (plus_utility-minus_utility) / (2*self.delta) # the math is trival
		return grad, i
	...
	def numerical_gradient_cons(self, m, cons,delta=1e-08):
		...
		res = pool.map(self._partial_grad_cons, indicies,cons)
		for g, i in res:
			grad[i] = g
		pool.close()
		pool.join()
