...
for j = 21:63
    ...
    title(['Utilities for Mitigation Changes at Node ','',num2str(j)]);
    ylabel('Utilities');
    xlabel(['Mitigation Level at Node ', '', num2str(j)]);
    u_summary = [u_summary,std(u_summary,1,2)];
    saveas(fig,['Utilities for Mitigation Changes at Node ','',num2str(j)],'epsc');
    save(['results_for_plot_','',num2str(j)]);
end
